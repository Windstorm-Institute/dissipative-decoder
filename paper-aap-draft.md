...
Serial decoding systems — ribosome, cognition, language, and artificial intelligence — converge to 3-6 bits per processing event. This study models serial decoding as maximizing net information return — bits gained minus discrimination cost — and shows that the size of the optimal alphabet is set by how discrimination cost scales. Two physically distinct cost regimes are identified. In Regime A (alphabet-bound), biological pairwise molecular recognition makes discrimination expensive and quadratic in alphabet size (cost ~ M(M-1)/2); the net-return optimum M* = (alpha * gamma * ln(2))^(-1/alpha) then falls near M = 20 amino acids for the estimated biological cost parameters, placing biology inside the 3-6 bit basin. In Regime B (capacity-bound), silicon discrimination is cheap and scales sub-linearly (alpha < 1, measured across 27 models), so the same objective places the optimum at a far larger alphabet and silicon is not confined to the basin. The optimum is set by the discrimination-cost structure, not a parameter-free constant. Empirical validation spans three substrates: the ribosome sits essentially on its informational (rate-distortion) floor (phi_info ~ 1.02, zero slack in bits) while thermodynamically dissipating roughly 25x the Landauer minimum, while a 7-billion-parameter transformer operates approximately 800 million times above its Landauer floor. The slack ordering across Regime A systems — biology nearest the floor, cognition highest — is consistent with the thermodynamic framework. A temperature prediction was tested against the live Kazusa Codon Usage Database: across 29 organisms spanning 8 to 100 degrees Celsius growth temperature, the partial correlation between amino acid entropy and temperature (controlling for GC-content confound) is r = -0.451 (p = 0.014), providing preliminary evidence that heat taxes information. The basin is not a universal speed limit but a speed limit for pairwise discriminators. Biology is one; silicon is not. The mathematics explains both regimes, resolving the apparent paradox of why AI lands in the biological throughput basin despite having no thermodynamic constraint on vocabulary size. This is the fifth paper in a nine-paper series on universal constraints in serial information processing.

**Keywords:** dissipative decoder, thermodynamic cost, discrimination energy, Landauer bound, kinetic proofreading, alphabet-bound, capacity-bound, ribosome efficiency, throughput basin, information theory
...
The relevant objective is the net information retained after paying discrimination cost (not the bare bits-per-joule ratio):

U(M) = log_2(M) - gamma * M^alpha                                                    (2)

Maximizing the bare ratio log_2(M) / (gamma * M^alpha) is the wrong objective: it peaks at M* = e^(1/alpha) ~ 1.6 for quadratic cost, degenerately favoring a two-symbol alphabet. For the net return U(M) = log_2(M) - gamma * M^alpha, both regimes have an interior optimum; what differs is where it falls. Expensive, steeply scaling discrimination (Regime A) puts the optimum at a small alphabet inside the 3-6 bit basin; cheap, sub-linearly scaling discrimination (Regime B) puts it at a large alphabet far above the basin.

### 2.4 Energy Benchmark: 27 AI Models

Energy consumption was measured for 27 AI models on standardized hardware (NVIDIA RTX 5090) using nvidia-smi GPU power polling at 10ms intervals. Models spanned the Pythia family (14M to 1.4B parameters) and additional architectures. The relationship between energy and parameter count was assessed via log-log regression.
...
The net-return objective U(M) = log_2(M) - gamma * M^alpha has a unique interior maximum for every alpha > 0, located at M* = (1 / (alpha * gamma * ln(2)))^(1/alpha). The optimum is therefore not parameter-free: it is fixed by the discrimination-cost structure (alpha, gamma). What separates the regimes is not the existence of an optimum — both regimes have one — but where it falls. For steep, expensive discrimination (large alpha and gamma, as in pairwise molecular recognition) M* is small and lies inside the 3-6 bit basin; for cheap, sub-linearly scaling discrimination (small alpha and gamma, as in learned silicon parameters) M* is large and lies far above it. There is no sharp alpha = 1 phase boundary; the optimum moves continuously with the cost structure. The empirically measurable question is therefore not "super-linear or not?" but "how expensive and how steeply scaling is discrimination?"

### 3.2 Silicon Energy Scaling

Across 27 models evaluated on the NVIDIA RTX 5090, silicon energy scales as E proportional to params^0.937 (r^2 = 0.960, log-log regression). The exponent is sub-linear (a preliminary fit gives alpha ~ 0.9; the precise value is provisional pending release of the full 27-model dataset, and the four disclosed Pythia points alone fit a shallower slope, but sub-linearity is what the argument requires and is robust). This is the opposite of biology's expensive quadratic scaling: cheap sub-linear cost places silicon's net-return optimum at a large alphabet, so AI is not confined to the basin. The consequence: no peak in bits-per-joule, no optimal vocabulary size, no thermodynamic basin. The smallest model is the most energy-efficient per bit of compression; efficiency decreases monotonically with scale.
...
For Regime A with quadratic discrimination cost (alpha ~ 2), the net-return optimum M* = (alpha * gamma * ln(2))^(-1/alpha) falls near M = 20 for the biophysically estimated cost coefficient gamma (see Limitations). On this reading the genetic code settled near 21 amino acids because that is where the marginal information from one more amino acid (~0.07 bit near M = 20) no longer justifies its quadratic discrimination overhead — not because M = 20 is a parameter-free constant. Self-replicating systems that use expensive pairwise molecular recognition converge to this neighborhood for cost coefficients in the biological range.
...
The net-return optimum near M ~ 20 has direct implications for efforts to expand the genetic code beyond the natural 20 amino acids. Adding a 22nd amino acid would require a new aminoacyl-tRNA synthetase capable of discriminating against all 21 existing amino acids — 21 new pairwise comparisons — for approximately 0.07 additional bits of information per codon. The thermodynamic cost-benefit ratio becomes increasingly unfavorable as M increases beyond 20. This analysis predicts that synthetic organisms with expanded genetic codes will exhibit reduced growth rates proportional to the discrimination cost overhead of the additional amino acids.
...
Grant Lavell Whitmer III conceived the two-regime framework, designed the energy benchmark, directed all computational experiments, analyzed all results, and prepared the manuscript.
...
[4] Landauer, R. "Irreversibility and heat generation in the computing process," IBM Journal of Research and Development, vol. 5, no. 3, pp. 183-191, 1961. DOI: 10.1147/rd.53.0183

[5] Zeldovich, K.B.; Berezovsky, I.N.; Shakhnovich, E.I. "Protein and DNA sequence determinants of thermophilic adaptation," PLoS Computational Biology, vol. 3, no. 1, pp. e5, 2007. DOI: 10.1371/journal.pcbi.0030005
...
[7] Hopfield, J.J. "Kinetic proofreading: A new mechanism for reducing errors in biosynthetic processes," Proceedings of the National Academy of Sciences, vol. 71, no. 10, pp. 4135-4139, 1974. DOI: 10.1073/pnas.71.10.4135
...
[10] Whitmer III, G.L. "Dissipative Decoder: Experiment Code and Data," GitHub, 2026. https://github.com/Windstorm-Labs/dissipative-decoder (accessed Apr. 12, 2026).
