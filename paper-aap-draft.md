...
Serial decoding systems — ribosome, cognition, language, and artificial intelligence — converge to 3-6 bits per processing event. This study derives a discrimination cost function with super-linear alphabet scaling and exponential fidelity scaling, showing that the throughput basin is geometrically inevitable for systems with super-linear discrimination cost. Two physically distinct cost regimes are identified. Regime A (alphabet-bound) describes biological systems using pairwise molecular recognition, where cost scales with M(M-1)/2 (quadratic), producing a thermodynamically constrained basin with a unique efficiency peak at approximately M = 20 amino acids. Regime B (capacity-bound) describes silicon AI systems using independent learned parameters, where cost scales sub-linearly with model capacity (measured exponent alpha = 0.937 across 27 models on standardized hardware), producing no cost-driven basin. Empirical validation spans three substrates: the ribosome sits essentially on its informational (rate-distortion) floor (phi_info ~ 1.02, zero slack in bits) while thermodynamically dissipating roughly 25x the Landauer minimum, while a 7-billion-parameter transformer operates approximately 800 million times above its Landauer floor. The slack ordering across Regime A systems — biology nearest the floor, cognition highest — is consistent with the thermodynamic framework. A temperature prediction was tested against the live Kazusa Codon Usage Database: across 29 organisms spanning 8 to 100 degrees Celsius growth temperature, the partial correlation between amino acid entropy and temperature (controlling for GC-content confound) is r = -0.451 (p = 0.014), providing preliminary evidence that heat taxes information. The basin is not a universal speed limit but a speed limit for pairwise discriminators. Biology is one; silicon is not. The mathematics explains both regimes, resolving the apparent paradox of why AI lands in the biological throughput basin despite having no thermodynamic constraint on vocabulary size. This is the fifth paper in a seven-paper series on universal constraints in serial information processing.
